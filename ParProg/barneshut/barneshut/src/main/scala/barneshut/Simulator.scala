...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics) {
  ...
  def updateBoundaries(boundaries: Boundaries, body: Body): Boundaries = {
    ...
    bounds.maxX = math.max(boundaries.maxX, body.x)
    bounds.minX = math.min(boundaries.minX, body.x)
    bounds.maxY = math.max(boundaries.maxY, body.y)
    bounds.minY = math.min(boundaries.minY, body.y)
    bounds
  }
