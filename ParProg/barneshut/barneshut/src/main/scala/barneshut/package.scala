...
package object barneshut {
  ...
  case class Fork(
    ...
    def insert(b: Body): Fork = {
      ...
      else new Fork(nw, ne.insert(b), sw, se)
    }
  }

  case class Leaf(centerX: Float, centerY: Float, size: Float, bodies: Seq[Body]) extends Quad {
    val (mass, massX, massY) = (bodies.aggregate(0.toFloat)((a: Float, b: Body) => a + b.mass, _ + _) : Float,
      bodies.aggregate(0.toFloat)((a: Float, b: Body) => a + b.x * b.mass, _ + _) / bodies.aggregate(0.toFloat)((a: Float, b: Body) => a + b.mass, _ + _) : Float,
        bodies.aggregate(0.toFloat)((a: Float, b: Body) => a + b.y * b.mass, _ + _) / bodies.aggregate(0.toFloat)((a: Float, b: Body) => a + b.mass, _ + _) : Float)
    val total: Int = bodies.length
    def insert(b: Body): Quad = {
      if (size > minimumSize) {
        var fork = new Fork(Empty(centerX - size/4, centerY - size/4, size/2),
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      val x = (math.max(math.min(b.x, boundaries.maxX), boundaries.minX) - boundaries.minX) / sectorSize
      val y = (math.max(math.min(b.y, boundaries.maxY), boundaries.minY) - boundaries.minY) / sectorSize
