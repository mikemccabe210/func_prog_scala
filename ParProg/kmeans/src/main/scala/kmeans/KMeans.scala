...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    var groups = points.groupBy((point: Point) => findClosest(point, means))
    for (m <- means){
      groups = groups.updated(m, groups.getOrElse(m, GenSeq[Point]()))
    }
    groups
  }
  ...
  def update(classified: GenMap[Point, GenSeq[Point]], oldMeans: GenSeq[Point]): GenSeq[Point] = {
    for (mean <- oldMeans) yield findAverage(mean, classified(mean))
  }

  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    var conv = true
    var i = 0
    while (conv && i < oldMeans.length){
      if (oldMeans(i).squareDistance(newMeans(i)) > eta) conv = false
      i += 1
    }
    conv
  }
